...
<body onload="initialize();">
    ...
    <script type="text/javascript">
        ...
        let endDate; // end timestamp of a question
        ...
        let checkQuestion = () => {
            ...
            if (obj[index]["datetime_answered"] === "") {
                ...
                if (numCurrentAttempts === (parseInt(obj[index]["numTries"]) - 1)) {
                    ...
                    //set the endDate to be in format (yyyy-mm-dd hh:mm:ss)
                    let date = new Date();
                    endDate = date.getFullYear() + "-" + ("0" + (date.getMonth() + 1)).slice(-2) + "-" + ("0" + date.getDate()).slice(-2) + " " + ("0" + date.getHours()).slice(-2) + ":" + ("0" + date.getMinutes()).slice(-2) + ":" + ("0" + date.getSeconds()).slice(-2);

                    // stop timer
                    stopTimer(timerID);

                    /* UPDATE USER JSON FILE */
                    updateData(obj[index]["pkey"], numCurrentAttempts, correct, obj[index]["tags"], startDate, endDate);
                }
        ...
        let updateData = (pkey, numCurrentAttempts, correct, tags, startDate, endDate) => {
            request = new XMLHttpRequest();
            request.open('POST', 'js-php/update_user_json.php', true);
            request.onreadystatechange = respond;
            request.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
            request.send(
                "pkey=" + pkey + "&numCurrentTries=" + numCurrentAttempts +
                "&correct=" + correct + "&tags=" + tags + "&startDate=" + startDate +
                "&endDate=" + endDate
            );
        }
        let respond = () => {
